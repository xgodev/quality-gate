...
EOF
  ...
  local v_fmt v_lint v_build v_test v_cov regressed=0 reglist=()
  ...
  for v_pair in "fmt:$v_fmt" "lint:$v_lint" "build:$v_build" "test fails:$v_test" "coverage:$v_cov"; do
    name="${v_pair%:*}"
    verdict="${v_pair#*:}"
...
render_json() {
  ...
  local base_cov="${13}" pr_cov="${14}" cov_margin="${15}"

  local v_fmt v_lint v_build v_test v_cov global_verdict=passed
  v_fmt=$(verdict_count "$base_fmt" "$pr_fmt")
  v_lint=$(verdict_count "$base_lint" "$pr_lint")
  v_build=$(verdict_count "$base_build" "$pr_build")
  v_test=$(verdict_count "$base_test" "$pr_test")
  ...
  for v in "$v_fmt" "$v_lint" "$v_build" "$v_test" "$v_cov"; do
