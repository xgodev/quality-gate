...
#
# nodejs/qg.sh -- Quality Gate para Node.js
#
...
show_help() {
  cat <<'EOF'
Uso: nodejs/qg.sh [--base <git-ref>] [opcoes]
     nodejs/qg.sh --detect
...
check_prereqs() {
  local missing=()
  command -v node >/dev/null 2>&1 || missing+=("node (Node.js 18+) -- instale: 'curl -fsSL https://deb.nodesource.com/setup_lts.x | sudo -E bash - && sudo apt install -y nodejs' (Linux) / 'brew install node' (macOS) (sem Node nenhuma metrica roda)")
  command -v npm >/dev/null 2>&1 || missing+=("npm (vem com Node.js) -- instale: 'sudo apt install -y nodejs npm' (Linux) / 'brew install node' (macOS) (sem npm nao ha resolucao de deps)")
  command -v npx >/dev/null 2>&1 || missing+=("npx (vem com Node.js) -- instale: 'sudo apt install -y nodejs npm' (Linux) / 'brew install node' (macOS) (sem npx prettier/eslint/tsc/c8 nao rodam)")
  ...
  # prettier, eslint e c8 sao instalados sob demanda via 'npx --yes' (cacheados em ~/.npm/_npx).
  # Nao validamos versao especifica -- depende do que o projeto-alvo usa.
...
if [ "$QG_ABSOLUTE_MODE" = "1" ]; then
  ...
  if ! qg_resolve_deps "." "$QG_LOG_DIR_ARG/abs-deps.log"; then
    echo "::error::falha ao resolver dependencias de nodejs -- ver $QG_LOG_DIR_ARG/abs-deps.log" >&2
    exit 2
  fi
...
if [ -z "$QG_BASELINE_DIR_ARG" ]; then
  QG_BASELINE_DIR_ARG="/tmp/qg-baseline-nodejs"
...
# Linguagem ausente no baseline (sentinela: package.json)
...
EOF
    ...
    echo "::warning::linguagem ausente no baseline (package.json nao encontrado em $QG_BASELINE_DIR_ARG) -- gate skipped" >&2
  fi
  exit 0
fi
...
if ! qg_resolve_deps "$QG_BASELINE_DIR_ARG" "$QG_LOG_DIR_ARG/base-deps.log"; then
  echo "::error::falha ao resolver dependencias de nodejs (baseline) -- ver $QG_LOG_DIR_ARG/base-deps.log" >&2
  exit 2
fi
if ! qg_resolve_deps "." "$QG_LOG_DIR_ARG/pr-deps.log"; then
  echo "::error::falha ao resolver dependencias de nodejs (PR) -- ver $QG_LOG_DIR_ARG/pr-deps.log" >&2
  exit 2
fi
echo "-- medindo base --" >&2
