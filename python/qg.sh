...
#
# python/qg.sh -- Quality Gate para Python
#
...
show_help() {
  cat <<'EOF'
Uso: python/qg.sh [--base <git-ref>] [opcoes]
     python/qg.sh --detect
...
check_prereqs() {
  local missing=()
  command -v python3 >/dev/null 2>&1 || missing+=("python3 (Python 3.10+) -- instale: 'apt install python3 python3-pip' (Linux) / 'brew install python' (macOS) (sem Python nao ha como medir o projeto)")
  command -v ruff >/dev/null 2>&1 || missing+=("ruff -- instale: 'pip install ruff' (Linux) / 'brew install ruff' (macOS) (sem ruff as metricas fmt/lint nao rodam)")
  command -v pytest >/dev/null 2>&1 || missing+=("pytest -- instale: 'pip install pytest pytest-cov' (Linux) / 'pip install pytest pytest-cov' (macOS) (sem pytest as metricas test/coverage nao rodam)")
  command -v radon >/dev/null 2>&1 || missing+=("radon -- instale: 'pip install radon' (Linux) / 'pip install radon' (macOS) (sem radon a metrica complexity nao roda)")
  ...
  # pytest-cov eh modulo importavel; nao tem binario. Detecta via python.
  if command -v python3 >/dev/null 2>&1; then
    python3 -c "import pytest_cov" 2>/dev/null || missing+=("pytest-cov -- instale: 'pip install pytest-cov' (Linux) / 'pip install pytest-cov' (macOS) (sem pytest-cov a metrica coverage nao roda)")
  fi
...
if [ "$QG_ABSOLUTE_MODE" = "1" ]; then
  ...
  if ! qg_resolve_deps "." "$QG_LOG_DIR_ARG/abs-deps.log"; then
    echo "::error::falha ao resolver dependencias de python -- ver $QG_LOG_DIR_ARG/abs-deps.log" >&2
    exit 2
  fi
...
=== Quality Gate (fast-path) ===
  branch:        $branch
  base ref:      $QG_BASE_REF_ARG
  scope:         nenhum arquivo Python tocado -- pulando gates pesados
...
if [ -z "$QG_BASELINE_DIR_ARG" ]; then
  QG_BASELINE_DIR_ARG="/tmp/qg-baseline-python"
...
# Linguagem ausente no baseline (sentinelas: pyproject.toml, setup.py, requirements.txt)
...
EOF
    ...
    echo "::warning::linguagem ausente no baseline (nenhum pyproject.toml/setup.py/setup.cfg/requirements*.txt em $QG_BASELINE_DIR_ARG) -- gate skipped" >&2
  fi
  exit 0
fi
...
if ! qg_resolve_deps "$QG_BASELINE_DIR_ARG" "$QG_LOG_DIR_ARG/base-deps.log"; then
  echo "::error::falha ao resolver dependencias de python (baseline) -- ver $QG_LOG_DIR_ARG/base-deps.log" >&2
  exit 2
fi
if ! qg_resolve_deps "." "$QG_LOG_DIR_ARG/pr-deps.log"; then
  echo "::error::falha ao resolver dependencias de python (PR) -- ver $QG_LOG_DIR_ARG/pr-deps.log" >&2
  exit 2
fi
echo "-- medindo base --" >&2
